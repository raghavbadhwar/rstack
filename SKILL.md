...
1. If the request is internet research across search, GitHub, social, video, web,
   RSS, career, or market platforms → invoke `/agent-reach`.
2. If it names Scrapling or requires adaptive, repeatable, browser-rendered,
   anti-bot, or multi-page extraction → invoke `/scrapling`.
3. If the request is about a browser, QA, dogfooding, screenshots, or inspecting a page
   (open a site, test a deploy, take a screenshot, check a flow visually) → invoke `/browse`.
4. Otherwise, route by the rules below. If nothing matches, answer directly.
...
- User asks for internet research, web/social/GitHub search, or a video/RSS lookup → invoke `/agent-reach`
- User asks for Scrapling, adaptive selectors, a bounded crawl, or protected-page extraction → invoke `/scrapling`
