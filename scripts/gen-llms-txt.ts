...
export async function generateLlmsTxt(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  lines.push('# gstack');
  lines.push('');
  lines.push("> RStack is Raghav's agentic engineering stack: AI coding skills, a fast headless browser binary, research routing, and a design CLI. This file indexes every capability so agents can discover and invoke them without crawling individual SKILL.md files.");
  lines.push('');
  ...
  lines.push('## More');
  lines.push('');
  lines.push('- Repository: https://github.com/raghavbadhwar/rstack');
